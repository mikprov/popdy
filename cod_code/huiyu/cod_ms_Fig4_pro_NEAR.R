...
# put all these data frames into list (to loop over)
# for now, remove: cod3M, cod3Ps, cod4x, cod2J3KL
#datalist <- list(Northsea,Coas,W_Baltic,
                 Faroe,NE_Arctic,Celtic,Iceland,
                 Irish,Kat,W_Scotland,NGulf,SGulf,
                 GB,GM,cod3NO)
...
NEARlist = as.list(rep(NA,length(datalist))) # store matrix of LSB vs F
names(NEARlist) = codNames
...
for (i in 1:length(datalist)) {
  ...
  NS = datalist[[i]] # select data for one population from list

  data = subset(NS,Yearclass>1959)  #yearclasses vary among stocks
  data = subset(data,Yearclass<1990)

  # load parms for dataset i
  source(file = paste('C:/Users/provo/Documents/GitHub/popdy/cod_pops/',codNames[i], '.r', sep=''))
  # this should load parms: L_inf, K, TEMP
  #L_inf = 77  #Table 2 of ms
  #K = 0.26
  #to = 0
  ...
  # create NEAR df, use NEAR to calculate LSB
  ...
  LEPdf = matrix(0,length(Age),length(F.halfmax))
    for(g in 1:length(F.halfmax)){ # step through each fishing level
      ...
      #LEPdf[,g] = NEAR[,2]*NEAR[,3]*NEAR[,8]
      LEPdf[,g] = NEAR[,2]*NEAR[,3]*0.2*NEAR[,8]
      
    }
  write.table(LEPdf,file=paste("C:/Users/provo/Documents/GitHub/popdy/cod_code/mikaelaLSB/",codNames[i],".txt",sep=""))
  LSBlist[[i]] = LEPdf
  NEARlist[[i]] = NEAR
  }

# After the for loop, I should have a massive list. Each element in list
# is a different population, a matrix of LSB vs F
